...
You can refer to the [Restore the State](../getting-started/Quick Tutorial.md#restore-the-state) section in [Quick Tutorial](../getting-started/Quick Tutorial.md) for more details about using these environment variables. You can either add those variables in the `.env` file or set them directly when you run the application in the terminal.
...
Those endpoints are wrapped into RPC methods in [ts/src/rpc.ts](https://github.com/DelphinusLab/zkwasm-mini-rollup/blob/main/ts/src/rpc.ts) which can be used by any client to interact with the zkWasm Mini Rollup Application and you can find usage examples in [Quick Tutorial](../getting-started/Quick Tutorial.md#player-class).
